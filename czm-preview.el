...
where it can be seen."
  ...
  (when-let ((env-start (czm-preview--valid-env-start)))
	...
	(setq czm-preview--active-env-start env-start)
...
(defun czm-preview--valid-env-start ()
...
If `czm-preview--active-env-start' is non-nil and equal to the
...
function being advised, and ARGS are its argument."
  ...
  (cl-destructuring-bind (snippet start end box counters tempdir place-opts) args
	...
	(beginning-of-line)
	(when-let ((env-start (czm-preview--valid-env-start)))
	  (when (eq env-start czm-preview--active-env-start)
	    (preview-clearout start end tempdir)
	    (setq czm-preview--active-env-start nil)
...
;; 	      (when (eq (overlay-get ov 'preview-state) 'inactive)
...
;; 		(push ov 'preview-disabled)
;; 		(message "GOT HERE!")
;; 		(if (and (eq (overlay-buffer ov) (current-buffer))
;; 			 (or (<= pt (overlay-start ov))
;; 			     (>= pt (overlay-end ov))))
;; 		    (preview-region (overlay-start ov) (overlay-end ov)))))))
;;     (when lst
...
(defun czm-preview--TeX-region-create (file region original offset)
